...
def processCategory(
  ...
  if !os.exists(promptPath) then
    Logger.error(s"Prompt file $promptPath not found. Skipping category $category.")
    return 0
        ...
        while snippetsGenerated < snippetsNeeded do
          ...
          snippets.zipWithIndex.foreach { case (snippet, index) =>
            val record = OutputLine(
              id = s"${category}_${totalSnippetsGenerated + index + 1}",
          ...
          Logger.info(s"Progress for topic '$topic': ${existingCount + snippetsGenerated}/${config.snippetsPerTopic}")
...
def processCategories(
  ...
  config.prompts.foldLeft(0) { case (total, (category, promptFilePath)) =>
    try
...
def run: Unit =
  ...
  val totalExpectedSnippets = config.prompts.size * topics.size * config.snippetsPerTopic
  Logger.info(
    s"Planning to generate up to $totalExpectedSnippets snippets (${config.prompts.size} categories x ${topics.size} topics x ${config.snippetsPerTopic} snippets per topic)"
  )
