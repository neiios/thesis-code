...
def build_model(vocab_size, num_classes):
    ...
    x = keras.layers.Embedding(input_dim=vocab_size, output_dim=EMBEDDING_DIM, mask_zero=True, name="embedding")(inputs)
    x = keras.layers.Bidirectional(
        keras.layers.LSTM(LSTM_UNITS, return_sequences=True, recurrent_dropout=0.2, name="lstm_1")
    )(x)
    x = keras.layers.Bidirectional(keras.layers.LSTM(LSTM_UNITS, name="lstm_2"))(x)
...
def main(args):
    ...
    try:
        ...
        plot_model(model, to_file=output_dir / "lstm_model_architecture.png", show_shapes=True, show_layer_names=True)
        print(f"Model architecture diagram saved to {output_dir / 'lstm_model_architecture.png'}")
    except ImportError:
    ...
    print(f"Class names saved to {output_dir / 'classes.json'}")

    plot_training_history(history, output_dir / "training_history.png")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train an LSTM model for code smell detection")
    parser.add_argument(
        "-i",
        "--input-file",
        type=Path,
